...
// 1.1
// meta-function: 1) a mapping; 2) no side effects; 3) can be invoked in
// compile-time
constexpr int fun(int a) { return a + 1; }

// using template instead of constexpr (after C++14)
template <int a> constexpr int fun_1 = a + 1;
...
template <typename T> struct Fun2 {
...
template <template <class name> class T1, typename T2> struct Fun2_ {
...
template <> struct Fun3_<true> {
  template <class T> using type = std::add_lvalue_reference<T>;
};

template <> struct Fun3_<false> {
  template <class T> using type = std::remove_reference<T>;
};
...
template <template <class> class... T> struct TemplateContainer;
template <template <class...> class... T> struct TemplateContainer2;

// 1.3

// meta-object and meta-scope

template <typename T, size_t N> struct Fun4_ {
private:
...
template <typename T> struct Wrapper {
  template <size_t N> using method = T[N];
};

template <typename T> struct Fun5_ {
  using type = Wrapper<std::remove_pointer_t<T>>;
};

template <typename T> using Fun5 = typename Fun5_<T>::type;
...
template <typename T> struct Fun6_ {
  constexpr static size_t value = 0;
};

template <> struct Fun6_<A> {
  constexpr static size_t value = 1;
};

template <> struct Fun6_<B> {
...
// 在C++中优雅地检测类型/表达式有效性： void_t & is_detected -
// SuperSodaSea的文章 - 知乎 https://zhuanlan.zhihu.com/p/26155469
namespace my_std {
...
constexpr size_t Accumulate<CutInput, Inputs...> =
    CutInput + Accumulate<Inputs...>;

// 2) Fold Expressions, C++17
template <size_t... values> constexpr size_t fun() {
  return (0 + ... + values);
}

// 3) Parameter pack expansion, C++11
template <size_t... I> struct Cont;

template <size_t... I> using Fun6 = Cont<(I + 1)...>;

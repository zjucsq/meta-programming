...
// C++的那些事——any类 - 船长J.K的文章 - 知乎
// https://zhuanlan.zhihu.com/p/589164471 C/C++杂谈：动手写个any - 月踏的文章 -
// 知乎 https://zhuanlan.zhihu.com/p/386392397
...
public:
  any()
      : value_(nullptr),
        ...
        clone_(nullptr), destroy_(nullptr) {}

  template <typename T>
  any(const T &value)
      : value_(static_cast<void *>(new T{value})),
        getType_([]() -> const std::type_info & { return typeid(T); }),
        clone_([](void *otherData) {
          return static_cast<void *>(new T{*static_cast<T *>(otherData)});
        }),
        destroy_([](void *data) { delete static_cast<T *>(data); }) {}

  any(const any &other)
      : value_(other.clone_(other.value_)), getType_(other.getType_),
  ...
  any(any &&other) noexcept
      : value_(other.value_), getType_(other.getType_), clone_(other.clone_),
        destroy_(other.destroy_) {
    other.clear();
  }
  ...
  template <typename T> T &any_cast() {
